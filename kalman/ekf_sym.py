...
def gen_code(name, f_sym, dt_sym, x_sym, obs_eqs, dim_x, dim_err, eskf_params=None, msckf_params=None, maha_test_kinds=[]):
    # Optional state transition matrix, H modifier and err_function if an error-state kalman filter (ESKF) is desired.
    # Best described in "Quaternion kinematics for the error-state Kalman filter" by Joan Sola
    if eskf_params:
...
class EKF_sym(object):
    def __init__(self, name, Q, x_initial, P_initial, dim_main, dim_main_err, N=0, dim_augment=0, dim_augment_err=0, maha_test_kinds=[]):
        ...
        # Kinds that should used with Mahalanobis distance
        ...
        # gen_code(name, f_sym, dt_sym, x_sym, obs_eqs, self.dim_x, self.dim_err)
